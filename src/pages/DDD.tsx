...
export function DDD() {
  ...
  const renderColumn = (type: TaskType, title: string, subtitle: string, gradient: string) => {
    ...
    return (
      <div className="flex-1 min-w-[340px] space-y-12">
        <div className={cn("p-10 rounded-[3rem] text-white shadow-[0_40px_80px_-20px_rgba(74,53,47,0.2)] relative overflow-hidden group border border-white/5", gradient)}>
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-6">
              <span className="text-[10px] font-bold uppercase tracking-[0.4em] opacity-40">{title}</span>
              <span className="bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] border border-white/10 group-hover:scale-105 transition-transform">{filteredTasks.length} ARQUIVOS</span>
            </div>
            <h2 className="text-4xl font-display font-bold tracking-tight uppercase leading-none">{subtitle}</h2>
          </div>
        ...
        <div className="space-y-5 px-2">
          <div className="flex items-center gap-4 p-4 bg-surface border border-surface-border rounded-3xl group focus-within:border-primary focus-within:shadow-2xl focus-within:shadow-primary/5 transition-all card-3d">
            <input 
              type="text" 
              ...
              className="flex-1 bg-transparent border-none outline-none px-6 text-[11px] font-bold uppercase tracking-[0.2em] placeholder:text-text-muted/40"
            />
            <button 
              onClick={() => addTask(type)}
              className="w-12 h-12 rounded-2xl bg-secondary text-white flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-xl shadow-secondary/30"
            >
          ...
          <div className="space-y-4">
            <AnimatePresence initial={false}>
              {filteredTasks.map((task) => (
                <motion.div 
                  ...
                  className="bg-surface border border-surface-border rounded-3xl p-6 flex items-center gap-6 group hover:border-primary/30 transition-all shadow-sm hover:shadow-2xl hover:shadow-primary/5 card-3d overflow-hidden"
                >
                  <button 
                  ...
                  <span className="flex-1 text-[11px] font-bold text-secondary uppercase tracking-widest truncate group-hover:text-primary transition-colors">{task.title}</span>
            ...
            {filteredTasks.length === 0 && (
              <div className="py-24 text-center border-2 border-dashed border-surface-border/50 rounded-[3rem] bg-surface/30">
                <div className="w-16 h-16 bg-surface-border/10 rounded-full flex items-center justify-center mx-auto mb-5">
                   <Target className="w-6 h-6 text-text-muted opacity-20" />
                </div>
                <p className="text-[10px] font-bold text-text-muted uppercase tracking-[0.5em] italic opacity-30">Mente limpa, <br />vácuo criativo.</p>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-16 pb-20">
      <header className="space-y-6">
        ...
        <p className="text-text-muted text-xl max-w-xl font-light">“A mente é para criar, não para armazenar. Esvazie o supérfluo, valide o essencial.”</p>
      </header>
